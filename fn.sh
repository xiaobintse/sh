...
get_user_input() {
  ...
  while true; do
    ...
    if [ -n "$P2P_ANNOUNCE_ADDRESS" ]; then
      ...
      if [ $? -eq 0 ]; then
        echo "有效的地址: $P2P_ANNOUNCE_ADDRESS"
        break
      else
        echo "无效的地址，请重新输入。"
      fi
...
generate_instance() {
  for ((i = START_NUM; i <= END_NUM; i++)); do
    ...
    # 动态生成 P2P_ANNOUNCE_ADDRESSES 格式
    if [[ "$P2P_ANNOUNCE_ADDRESS" =~ ^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
      # 如果是 IP 地址
      P2P_ANNOUNCE_FORMAT="/ip4/$P2P_ANNOUNCE_ADDRESS"
    else
      # 如果是 FQDN
      P2P_ANNOUNCE_FORMAT="/dns4/$P2P_ANNOUNCE_ADDRESS"
    fi
...
services:
  ocean-node:
    ...
    environment:
      ...
      P2P_ANNOUNCE_ADDRESSES: '["$P2P_ANNOUNCE_FORMAT/tcp/$P2P_TCP_PORT", "$P2P_ANNOUNCE_FORMAT/ws/tcp/$P2P_WS_PORT"]'
      DASHBOARD: 'true'
    networks:
      - ocean_network
    depends_on:

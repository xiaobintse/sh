...
function install_ritual_node() {
    ...
    if [ -f "$DOCKER_COMPOSE_FILE" ]; then
        # 修改 4000 为 4005
        sed -i 's/0.0.0.0:6666:6667/0.0.0.0:6666:6667/' "$DOCKER_COMPOSE_FILE" >> "$LOG_FILE" 2>&1
        # 修改 8545 为 8550
        sed -i 's/6669:7000/6669:7000/' "$DOCKER_COMPOSE_FILE" >> "$LOG_FILE" 2>&1
        echo "[提示] 已将端口映射修改为 0.0.0.0:6666:6667 和 6669*:7000" | tee -a "$LOG_FILE"
    else

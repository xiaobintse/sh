...
read -p "请输入您的验证者详情（例如'风男真帅'）: " details
...
screen -dmS ogcc ../target/release/zgs_node --config config.toml
...
screen -dmS ogkv ../target/release/zgs_kv --config config.toml
...
cd 0g-storage-node/run && screen -dmS ogcc ../target/release/zgs_node --config config.toml
...
function main_menu() {
    while true; do
        clear
        echo "风男提示你脚本免费开源，请勿相信任何其他人的收费，微信4561310"
        echo "=======================验证节点功能================================"
